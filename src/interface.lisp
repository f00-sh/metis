  ...
  (let* ((m (or mind (sess-mind sess)))
         ...
         (math (unless chat
                 (if (fboundp 'symbol-math-answer)
                     ...
                     (%iface-math-answer q))))
         (local (unless (or chat math)
                  (and (fboundp 'symbol-local-user-answer)
                       (symbol-local-user-answer q m))))
         (extractive (unless (or chat math local)
                       (iface-extractive-answer sess q)))
         (concept (unless (or chat math local extractive)
                    (if (fboundp 'symbol-nl-concept)
                        (symbol-nl-concept q)
                        (%iface-concept-answer q))))
         (kb-ans (unless (or chat math local extractive concept)
                   (%iface-kb-about m q)))
         (kb-eng (unless (or chat math local extractive concept kb-ans)
