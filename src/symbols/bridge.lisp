...
(defun %iface-symbol-commands (text)
  (cond
    ...
    ((cl-ppcre:scan "(?i)^/marketplace(?:\\s+list)?\\s*$" text)
     (list :marketplace-list))
    ((cl-ppcre:scan "(?i)^/marketplace\\s+install\\s+(\\S+)(?:\\s+(\\S+))?\\s*$" text)
     (cl-ppcre:register-groups-bind (src id)
         ("(?i)^/marketplace\\s+install\\s+(\\S+)(?:\\s+(\\S+))?\\s*$" text)
       (list :marketplace-install src id)))
